...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        a, b = self.names_to_nodes[a], self.names_to_nodes[b]
        # traverse up 
        ...
        if a == self.root:
            ...
            while a != self.root and a.get_parent() != self.root:
                dist_a += 1
                a = a.get_parent()
        ...
        if b == self.root:
            ...
            while b != self.root and b.get_parent() != self.root:
                dist_b += 1
                b = b.get_parent()
        if cous_type != -1:
            cous_type = min(dist_a, dist_b)            

        deg_removed = abs(dist_b - dist_a)

        return (cous_type, deg_removed)        
